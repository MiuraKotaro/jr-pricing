...
public class 料金計算サービス {
    public 料金 料金計算(駅 降車駅, 利用日 _利用日, 片道往復 _片道往復, 人数 大人人数, 人数 子供人数, 特急利用 _特急利用) {
        ...
        if (大人人数.大なりイコール(new 人数(1))) {
            料金 大人一人分片道料金 = 大人一人分片道計算(降車駅, _特急利用, _片道往復, _利用日, 大人人数, 子供人数);
            片道合計金額 = 大人一人分片道料金.かける(大人人数);
        }
        if (子供人数.大なりイコール(new 人数(1))) {
            料金 子供一人分片道料金 = 子供一人分片道計算(降車駅, _特急利用, _片道往復, _利用日, 大人人数, 子供人数);
            片道合計金額 = 子供一人分片道料金.かける(大人人数);
        }

        return _片道往復.is片道() ? 片道合計金額 : 片道合計金額.足す(片道合計金額);
    }

    private 料金 大人一人分片道計算(駅 降車駅, 特急利用 _特急利用, 片道往復 _片道往復, 利用日 _利用日, 人数 大人人数, 人数 子供人数) {
        料金 大人一人分運賃 = 降車駅.get運賃();
        大人一人分運賃 = 運賃割引計算(大人一人分運賃, 降車駅.get_距離(), _片道往復, _利用日, 大人人数, 子供人数);
        料金 大人一人分特急料金 = _特急利用.get特急料金(降車駅);
        料金 大人一人分 = 大人一人分運賃.足す(大人一人分特急料金);
        return 大人一人分.足す(_特急利用.get特急料金(降車駅));
    }

    private 料金 子供一人分片道計算(駅 降車駅, 特急利用 _特急利用, 片道往復 _片道往復, 利用日 _利用日, 人数 大人人数, 人数 子供人数) {
        final 割引率 子供割引率 = new 割引率(50);

        料金 子供一人分運賃 = 降車駅.get運賃().割引(子供割引率);
        子供一人分運賃 = 運賃割引計算(子供一人分運賃, 降車駅.get_距離(), _片道往復, _利用日, 大人人数, 子供人数);
        料金 子供一人分特急料金 = _特急利用.get特急料金(降車駅);
        料金 子供一人分 = 子供一人分運賃.足す(子供一人分特急料金);
        return 子供一人分.足す(_特急利用.get特急料金(降車駅).割引(子供割引率));
    }

    private 料金 運賃割引計算(料金 一人分片道運賃, 距離 _距離, 片道往復 _片道往復, 利用日 _利用日, 人数 大人人数, 人数 子供人数) {
        一人分片道運賃 = 往復割引計算(一人分片道運賃, _距離, _片道往復);
        一人分片道運賃 = 団体時期割引計算(一人分片道運賃, _利用日, 大人人数, 子供人数);
        return 一人分片道運賃;
    }

    private 料金 往復割引計算(料金 一人分片道運賃, 距離 _距離, 片道往復 _片道往復) {
        if (_片道往復.is片道()) {
            return 一人分片道運賃;
        }

        final 距離 往復割引最短距離 = new 距離(600);
        final 割引率 往復割引率 = new 割引率(10);
        return _距離.大なり(往復割引最短距離) ? 一人分片道運賃.割引(往復割引率) : 一人分片道運賃;
    }
